...
class TorchTransformerEncoder(torch.nn.Module):
    def __init__(self, d_model, num_heads, d_ff, num_layers):
        ...
        self.blocks = torch.nn.ModuleList(
            [TorchTransformerBlock(d_model, num_heads, d_ff) for _ in range(num_layers)]
        )
...
def sync_kernel_encoder_from_torch(kernel_encoder, torch_encoder):
    # likely kernel encoder stores blocks in .layers or .blocks
    if hasattr(kernel_encoder, "layers"):
        kernel_blocks = kernel_encoder.layers
    elif hasattr(kernel_encoder, "blocks"):
        kernel_blocks = kernel_encoder.blocks
    else:
        raise AssertionError("Could not find block container on kernel encoder")

    assert len(kernel_blocks) == len(torch_encoder.blocks)

    for kb, tb in zip(kernel_blocks, torch_encoder.blocks):
        sync_kernel_block_from_torch(kb, tb)


def test_transformer_encoder_forward_parity_with_torch():
    np.random.seed(42)
    ...
    torch_encoder = TorchTransformerEncoder(d_model, num_heads, d_ff, num_layers)
    kernel_encoder = TransformerEncoder(
        d_model=d_model,
        num_heads=num_heads,
        d_ff=d_ff,
    ...
    sync_kernel_encoder_from_torch(kernel_encoder, torch_encoder)

    x_torch = torch.tensor(x_np, dtype=torch.float32)
    out_torch = torch_encoder(x_torch).detach().cpu().numpy()

    x_kernel = Tensor(x_np)
    out_kernel = maybe_first(kernel_encoder(x_kernel))
...
def test_transformer_encoder_forward_parity_with_torch_causal_mask():
    np.random.seed(42)
    torch.manual_seed(42)
    ...
    x_torch = torch.tensor(x_np, dtype=torch.float32)
    out_torch = torch_encoder(x_torch, attn_mask=torch_mask).detach().cpu().numpy()

    x_kernel = Tensor(x_np)
    out_kernel = maybe_first(kernel_encoder(x_kernel, mask=kernel_mask))
    out_kernel_np = to_numpy(out_kernel)
...
def test_transformer_encoder_backward_input_grad_parity_with_torch():
    np.random.seed(42)
    torch.manual_seed(42)
    ...
    out_torch = torch_encoder(x_torch)
