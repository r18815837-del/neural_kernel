...
def main():
    ...
    print("start token classifier CLS training demo")
    first_loss = None
    last_loss = None
    ...
    for step in range(1, 81):
        ...
        if step % 10 == 0 or step == 1:
            cls_grad_ok = model.cls_token.grad is not None
            print(
                f"step={step:03d}  loss={loss_value:.6f}  "
                f"acc={acc_value:.3f}  cls_grad={cls_grad_ok}"
            )
    ...
    if model.cls_token.grad is None:
        raise AssertionError("CLS token did not receive gradients")
    ...
    print("[OK] token classifier CLS training demo")
