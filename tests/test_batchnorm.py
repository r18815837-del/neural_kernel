...
from kernel.nn.normalization import BatchNorm1d


def test_batchnorm1d_train_updates_running_stats():
    np.random.seed(42)
...
def test_batchnorm1d_eval_uses_running_stats():
    ...
    x_train = Tensor(np.random.randn(8, 4), requires_grad=False)
    _ = bn(x_train)
    ...
    x_test = Tensor(np.random.randn(8, 4), requires_grad=False)
    y = bn(x_test)
